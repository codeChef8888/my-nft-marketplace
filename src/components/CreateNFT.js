...
const Create = ({ marketPlace, nft }) => {
  ...
  const createNFT = async () => {
    ...
    try {
      ...
      const result = await client.add(JSON.stringify({ image, price, name, description })); //Adding metadata in JSON with Object containing metadata.
  ...
  const mintThenList = async (result) => {
    ...
    if (nft != null) {
      ...
      //Mint the NFT 
      await nft.methods.mint(uri).send({ from: account }).on('transactionHash', (hash) => {
