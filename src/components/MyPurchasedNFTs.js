...
export default function MyPurchases({ marketPlace, nft, nft1155, web3 }) {
  ...
  const loadPurchasedNFTs = async () => {
    ...
    const result = await marketPlace
      .getPastEvents("Bought", filterOptions)
      .then((results) => {
        getNFTs(results);
      })
    ...
    const purchases = await Promise.all(
      results.map(async (payload) => {
        ...
        try {
          ...
          console.log(
            e.message,
            "The Error On loading NFT721 based on user!!!"
          );
        }
      })
    );
  ...
  const loadPurchasedNFT1155s = async () => {
    ...
    const result = await marketPlace
      .getPastEvents("Bought1155", filterOptions)
      .then((results) => {
        getNFT1155s(results);
      })
      .catch((err) => console.log(err));
  ...
  async function getNFT1155s(results) {
    ...
    const purchases1155 = await Promise.all(
      results.map(async (payload) => {
        // fetch arguments from each result
        //    i.events.Bought.returnValues[];
        let item = payload.returnValues;
        ...
        try {
          ...
          //  get total price of item (item price + fee)
          const totalPrice = await marketPlace.methods
            .getTotalPrice1155(item.itemId, item.amount)
            .call();
          ...
          console.log(
            e.message,
            "The Error Msg On Loading Purchased NFT1155 by Logged In User!!!"
          );
        }
      })
    );
                ...
                <Col key={idx} className="overflow-hidden">
                  <Card>
                    ...
                    <Card.Body color="secondary">
                      <Card.Text>No. of NFTs: {item.amount}</Card.Text>
                    </Card.Body>
                    <Card.Footer>
                      {web3.utils.fromWei(item.totalPrice, "ether")} ETH
                    </Card.Footer>
