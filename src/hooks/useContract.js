...
export const useNFT = () => {
  ...
  const tokenID = () => nft.methods.tokenCount().call(); // to get tokenId of new nft

  const setApprovalForAll = (account) =>
    nft.methods
      .setApprovalForAll(getMarketPlaceAddress(), true)
      .send({ from: account })
      .on("transactionHash", (hash) => {
        console.log("la bhayou APPROVAL");
      });

  const mintNFT = (uri, account) =>
    nft.methods
      .mint(uri)
      .send({ from: account })
      .on("transactionHash", (hash) => {
  ...
  return { nft, tokenURI721, tokenID, mintNFT, setApprovalForAll };
};
...
export const useMarketPlace = () => {
  ...
  const getTotalPrice721 = (itemID) =>
    marketPlace.methods.getTotalPrice(itemID).call();
  const getTotalPrice1155 = (item, amount) =>
    marketPlace.methods.getTotalPrice1155(item.itemId, amount).call();

  const mintThenList721 = async (result, price, account) => {
    ...
    try {
      const { mintNFT, tokenID, setApprovalForAll } = useNFT();
      //Mint the NFT
      await mintNFT(uri, account);
      // get tokenId of new nft
      const tokenId = await tokenID();
      await setApprovalForAll(account)
        .then(async () => await make721(account, price, tokenId))
        .catch((error) =>
          console.log(error.message, "CREATE NFT 721 ERROR!!!")
        );
    } catch (e) {
      console.log(e.message, "Error On MINT THEN LIST!!!!");
    }
  };

  const make721 = (account, price, tokenID) =>
    marketPlace.methods
      .makeItem(getNFTAddress(), tokenID, toWEI(price))
      .send({ from: account })
      .on("transactionHash", (hash) => {
        console.log("hum Idharr");
        console.log("la banyou tmro NFT lai");
      });

  const buy721 = (item, account) =>
    marketPlace.methods
      .purchaseItem(item.itemId)
      .send({ from: account, value: item.totalPrice })
      .on("transactionHash", (hash) => {
        console.log(hash, "nft721 purchased transaction hash...");
      });
  const buy1155 = (item, amount, totalAmount, account) =>
    marketPlace.methods
      .purchaseItem1155(item.itemId, amount)
    ...
    try {
      ...
      for (let i = 1; i <= count; i++) {
        ...
        if (!item.sold) {
          ...
          console.log(
            [item.itemid, metadata.name, metadata.image],
            "mero returned vava"
          );
          items.push({
            totalPrice,
      ...
      console.log(e.message, "Error From loadMarketPlaceItems721");
    }
    return items;
  };
